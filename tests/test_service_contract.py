...
def test_search_uses_optional_response_and_resolve_uses_only_response():
    ...
    search_block = registrations[search_start:resolve_start]
    resolve_block = registrations[resolve_start:subtitle_start]
    assert "SupportsResponse.OPTIONAL" in search_block
    assert "SupportsResponse.ONLY" in resolve_block


def test_play_handler_still_delegates_to_existing_playback_pipeline():
    tree = ast.parse(_source())
    setup = next(
        node for node in tree.body if isinstance(node, ast.AsyncFunctionDef) and node.name == "async_setup"
    )
    play = next(
        node for node in setup.body if isinstance(node, ast.AsyncFunctionDef) and node.name == "handle_play"
    )
    calls = {
        node.func.id
...
def test_manifest_is_051_without_config_entry_migration_increment():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    assert manifest["version"] == "0.5.1"
    assert "VERSION = 8" in (
        ROOT / "custom_components" / "stremio_stream_bridge" / "config_flow.py"
